defmodule AidaWeb.Channel.WebSocketTest do
  ...
  test "fail to join if the bot doesn't exist" do
    assert {:error, %{reason: "unauthorized"}} =
             socket()
             |> subscribe_and_join(BotChannel, "bot:2f6dc44d-dad1-4969-b22f-729c3b77f638", %{
               "access_token" => "1234"
             })
  end
  ...
  test "push data to be merged in the session", %{bot: bot} do
    ...
    socket
    |> push("put_data", %{"session" => session_id, "data" => new_data})
    |> assert_reply(:ok, _)
  ...
  describe "images" do
    ...
    test "bot answers a utb image message", %{
      image: %{uuid: image_uuid, session_id: session_id},
      socket: socket
    } do
    ...
    test "bot answers a utb image message with no image", %{
      ...
      assert(
        MessageLog
        |> Repo.get_by!(content: "invalid_image:BBBBBBBB-BBBB-BBBB-BBBB-BBBBBBBBBBBB")
      )
    end

    test "bot answers a utb image from a different session with no image", %{
      ...
      assert_push(
        "btu_msg",
        %{session: ^new_session_id, text: "Can you send me a picture?"},
        1000
      )
