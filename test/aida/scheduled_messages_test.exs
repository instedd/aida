defmodule Aida.ScheduledMessagesTest do
  alias Aida.{Skill.ScheduledMessages, Message, Bot, DB.MessageLog, Skill, Session, SessionStore, BotManager, Recurrence, DB}
  ...
  describe "schedule messages since last incoming message" do
    ...
    test "wake up after first delay", %{session_id: session_id, bot: bot, skill: skill} do
      message_log = MessageLog.create(%{bot_id: bot.id, session_id: session_id, session_uuid: @session_uuid, direction: "incoming", content: "Hi", content_type: "text"})
    ...
    test "wake up after second delay", %{session_id: session_id, bot: bot, skill: skill} do
      MessageLog.create(%{bot_id: bot.id, session_id: session_id, session_uuid: @session_uuid, direction: "incoming", content: "Hi", content_type: "text"})
    ...
    test "don't send if the language is not set", %{session: session, bot: bot, skill: skill} do
      message_log = MessageLog.create(%{bot_id: bot.id, session_id: session.id, session_uuid: @session_uuid, direction: "incoming", content: "Hi", content_type: "text"})
      wake_up_ts = within(minutes: 60)
      next_ts = Timex.shift(message_log.inserted_at, minutes: 1440)
