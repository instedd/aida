defmodule Aida.DecisionTreeTest do
  ...
  @basic_answer %{
    "answer" => %{"en" => "Go with an ice cream", "es" => "Comete un helado"},
  ...
  @basic_tree %{
    ...
    "responses" => [
        ...
        "next" => %{
          ...
          "responses" => [
            %{
              ...
              "next" => %{
                "answer" => %{"en" => "Go with Risotto", "es" => "Clavate un risotto"},
            ...
            %{
              ...
              "next" => %{
                "answer" => %{"en" => "Go with barbecue", "es" => "Comete un asado"},
      ...
      %{"keywords" => %{"en" => ["dessert"], "es" => ["postre"]}, "next" => @basic_answer}
    ]
  }
  ...
  describe "flatten" do
    test "simple flatten" do
      ...
      is_vegetarian =
        flattened["c038e08e-6095-4897-9184-eae929aba8c6"].responses
  ...
  describe "parse" do
    test "parse question" do
      ...
      main_course =
        question.responses
        |> Enum.find(
          &Enum.any?(Map.values(&1.keywords), fn x -> Enum.member?(x, "main course") end)
        )

      dessert =
        question.responses |> Enum.find(&Enum.member?(Map.values(&1.keywords), ["dessert"]))
    ...
    test "parse question stores responses in downcase" do
      ...
      main_course =
        question.responses
        |> Enum.find(&Enum.any?(Map.values(&1.keywords), fn x -> Enum.member?(x, "main") end))
  ...
  describe "runtime" do
    setup do
      ...
      session =
        Session.new({bot.id, "facebook", "1234567890/0987654321"})
        |> Session.save()
    ...
    test "starts the DecisionTree when the kewyord matches", %{bot: bot, session: session} do
      ...
      assert message.session
             |> Session.get_value(".decision_tree/2a516ba3-2e7b-48bf-b4c0-9b8cd55e003f") == %{
               "question" => "c5cc5c83-922b-428b-ad84-98a5c4da64e8"
             }
    end

    test "accepts an answer for the root question and performs the next", %{
      bot: bot,
      session: session
    } do
      ...
      message =
        message
        |> Message.put_session(".decision_tree/2a516ba3-2e7b-48bf-b4c0-9b8cd55e003f", %{
          "question" => "c5cc5c83-922b-428b-ad84-98a5c4da64e8"
        })
      ...
      assert message.session
             |> Session.get_value(".decision_tree/2a516ba3-2e7b-48bf-b4c0-9b8cd55e003f") == %{
               "question" => "42cc898f-42c3-4d39-84a3-651dbf7dfd5b"
             }
    end

    test "accepts an answer for the root question when there are different ways of selecting a branch (array) and performs the next",
         %{bot: bot, session: session} do
      ...
      assert message.session
             |> Session.get_value(".decision_tree/2a516ba3-2e7b-48bf-b4c0-9b8cd55e003f") == %{
               "question" => "c038e08e-6095-4897-9184-eae929aba8c6"
             }
    end

    test "when there is an invalid answer performs the last question again", %{
      bot: bot,
      session: session
    } do
    ...
    test "when it reaches an answer replies with it and resets the session", %{
      bot: bot,
      session: session
    } do
      ...
      assert message.session
             |> Session.get_value(".decision_tree/2a516ba3-2e7b-48bf-b4c0-9b8cd55e003f") == nil
    end
    ...
    test "clears the session when another skill takes over", %{bot: bot, session: session} do
      ...
      assert message |> Message.get_session(".decision_tree/2a516ba3-2e7b-48bf-b4c0-9b8cd55e003f") ==
               nil
    end

    test "each decision tree only clears its own session key but only if another skill is triggered",
         %{bot: bot, session: session} do
      session =
        session
        |> Session.merge(%{
