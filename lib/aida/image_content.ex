defmodule Aida.Message.ImageContent do
  ...
  def pull_and_store_image(%ImageContent{source_url: source_url, image_id: nil} = content, bot_id, session_id) do
    ...
    if source_url != "" do
      ...
      binary_type = case headers |> Enum.filter(&(&1 |> elem(0) == "Content-Type")) |> hd do
                      {"Content-Type", content_type} -> content_type
                      _ -> "image/jpeg"
                    end
      {:ok, db_image} = DB.create_image(%{binary: body, binary_type: binary_type, source_url: source_url, bot_id: bot_id, session_id: session_id})
