defmodule Aida.Skill.HumanOverride do
  ...
  defimpl Aida.Skill, for: __MODULE__ do
    ...
    def day_matches?(%{"day" => day}, weekday),
      do: String.downcase(day) == String.downcase(weekday)

    def parse_date(:beginning_of_day, day_reference),
      do: Timex.set(day_reference, hour: 0, minute: 0, second: 0)

    def parse_date(:end_of_day, day_reference),
      do: Timex.set(day_reference, hour: 23, minute: 59, second: 59)

    def parse_date(string, day_reference) do
      %{"hour" => hour, "minute" => minute} =
        Regex.named_captures(~r/(?<hour>\d+):(?<minute>\d+)/, string)
    ...
    def time_in_hours?(%{"since" => since, "until" => until}, timestamp) do
      timestamp
      |> Timex.between?(
        parse_date(since, timestamp),
        parse_date(until, timestamp),
        inclusive: true
      )
    end

    def time_in_hours?(%{"since" => since}, timestamp),
      do: time_in_hours?(%{"since" => since, "until" => :end_of_day}, timestamp)

    def time_in_hours?(%{"until" => until}, timestamp),
      do: time_in_hours?(%{"since" => :beginning_of_day, "until" => until}, timestamp)

    def time_in_hours?(%{}, timestamp),
      do: time_in_hours?(%{"since" => :beginning_of_day, "until" => :end_of_day}, timestamp)
    ...
    def in_hours?(timestamp, %{"hours" => hours, "timezone" => timezone}) do
      ...
      is_in_hours =
        hours
        |> Enum.any?(fn hour ->
          day_matches?(hour, local_weekday) and time_in_hours?(hour, local_timestamp)
        end)
    ...
    def put_response(skill, %{bot: bot, timestamp: timestamp} = message) do
      ...
      Bot.notify(bot, :human_override, %{
        message: Message.text_content(message),
        bot_id: bot.id,
        session_id: message.session.id,
        name: Message.get_session(message, "first_name")
      })
